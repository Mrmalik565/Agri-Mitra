...
const getAiInstance = (): GoogleGenAI => {
    if (!ai) {
        // Vite exposes env variables via import.meta.env, but the define plugin
        // in vite.config.ts replaces process.env.API_KEY with the actual value at build time.
        const apiKey = process.env.API_KEY;
        if (!apiKey) {
            console.error("API_KEY not found. Ensure it is set in GitHub Secrets as VITE_API_KEY and the build workflow is correct.");
