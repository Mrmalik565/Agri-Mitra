...
// --- Settings Context (replaces ThemeContext) ---
...
const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <SettingsContext.Provider value={{ theme, setTheme, voice, setVoice: handleSetVoice }}>
      {children}
        ...
        <div className="min-h-screen w-full flex flex-col items-center justify-center bg-cover bg-center bg-fixed p-4" style={{backgroundImage: "url('https://images.unsplash.com/photo-1625246333195-78d9c38ad449?q=80&w=2070&auto=format&fit=crop')"}}>
            ...
            >
                ...
                <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
                    <motion.button 
                        onClick={onLaunch} 
                        className="px-8 py-4 bg-emerald-600 text-white font-bold rounded-lg shadow-lg text-lg transition-transform"
                    ...
                    >
                     ...
                     <motion.a
                        ...
                        className="px-8 py-4 bg-white/20 border border-white/50 text-white font-bold rounded-lg shadow-lg text-lg transition-transform"
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                    >
                        {t('downloadApk')}
...
const AppContent: React.FC = () => {
  ...
  if (isInitializing) {
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-emerald-50 text-emerald-800 dark:bg-slate-900 dark:text-emerald-300">
        <KisanDoctorLogo className="h-28 w-28 animate-pulse" />
        <h1 className="text-4xl font-bold mt-4">Kisan Mitra</h1>
        <p className="mt-2 text-lg">Your AI Farming Assistant</p>
      </div>
    );
  }
  ...
  return (
      <AnimatePresence mode="wait">
        {appState === 'landing' && (
            <motion.div key="landing" exit={{ opacity: 0, scale: 0.9 }} transition={{ duration: 0.5 }}>
